...
@section('content')
    ...
    @endcomponent
    <div class="col-12 text-danger mt-3" id='errors'>

    </div>
    <livewire:home-slider view='edit' />
...
@section('script')
    ...
    <script>
        ...
        function emitEvent() {
            var es = $('#ui');
            var arr = {};
            es.children().each(function(ind, val) {
                arr[$(val).data('id')] = $(val).index();
            })
            // Here You Will Update Sorted Elements Of Slider Every Time
            Livewire.emit('update_list', arr);
        }
        ...
        function saveSlider() {
            ...
            var page = $('#slide_page').val();
            var status = $('#slide_status').val();
            if (!cropper) {
                /* EDIT WITH OUT IMAGE */
                $.ajax({
                    type: "POST",
